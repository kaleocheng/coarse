...
const coarse = (input, options = {}) => {
  ...
  const allChildren = svg.querySelectorAll('circle, rect, ellipse, line, polygon, polyline, path');
  const children = Array.from(allChildren).filter(
    node => node.tagName !== 'path' || node.parentNode.parentNode.tagName !== 'g'
  );
